...
init -1 style edited_nobreak is default:
    kerning 8
    outlines [(10, "#000", 0, 0)]
    xpos gui.text_xpos
    xanchor gui.text_xalign
    ...
    ypos gui.text_ypos
    text_align gui.text_xalign
    layout ("subtitle" if gui.text_xalign else "nobreak")
        ...
        if not persistent.autoload or not main_menu:
            ...
            if main_menu:
                ...
                if persistent.playthrough == 1:
                    textbutton _("ŔŗñĮ¼»ŧþŀÂŻŕěōì«") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="이름을 입력해 주세요", ok_action=Function(FinishEnterName)))
                else:
                    textbutton _("새 게임") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="이름을 입력해 주세요", ok_action=Function(FinishEnterName)))

            else:

                textbutton _("기록") action [ShowMenu("history"), SensitiveIf(renpy.get_screen("history") == None)]

                textbutton _("게임 저장") action [ShowMenu("save"), SensitiveIf(renpy.get_screen("save") == None)]

            textbutton _("게임 불러오기") action [ShowMenu("load"), SensitiveIf(renpy.get_screen("load") == None)]
            ...
            if _in_replay:
                ...
                if persistent.playthrough != 3:
                    ...
                    textbutton _("메인 메뉴") action NullAction()

            textbutton _("설정") action [ShowMenu("preferences"), SensitiveIf(renpy.get_screen("preferences") == None)]
            ...
            if renpy.variant("pc"):
                ...
                textbutton _("도움말") action [Help("README.html"), Show(screen="dialog", message="인터넷 브라우저로 도움말 파일을 열었습니다.", ok_action=Hide("dialog"))]
...
init -1 python:
    def FileActionMod(name, page=None, **kwargs):
        if persistent.playthrough == 1 and not persistent.deleted_saves and renpy.current_screen().screen_name[0] == "load" and FileLoadable(name):
            return Show(screen="dialog", message="파일 오류: \"characters/sayori.chr\"\n\n파일이 손상되었거나 존재하지 않습니다.",
                ok_action=Show(screen="dialog", message="저장된 데이터가 손상되었습니다. 새 게임을 시작합니다.", ok_action=Function(renpy.full_restart, label="start")))
        elif persistent.playthrough == 3 and renpy.current_screen().screen_name[0] == "save":
            return Show(screen="dialog", message="저장하는 의미가 없지 않아?\n걱정마. 나 어디 안 가니까.", ok_action=Hide("dialog"))
        else:
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:
                        ...
                        text FileTime(slot, format=_("{#file_time}%A, %B %d %Y, %H:%M"), empty=_("빈 공간")):
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    style_prefix "check"
                    label _("텍스트 넘기기")
                    textbutton _("선택지까지") action Preference("skip", "toggle")
                    textbutton _("한 번 본 텍스트") action Preference("after choices", "toggle")
...
init -501 screen history() tag menu:
    ...
    use game_menu(_("기록"), scroll=("vpgrid" if gui.history_height else "viewport")):
    ...
    frame:
        ...
        if in_sayori_kill and message == layout.QUIT:
            ...
            if message == layout.QUIT:
                label _("정말로 게임을 종료하시겠습니까?"):
                    style "confirm_prompt"
                    xalign 0.5
            elif message == layout.DELETE_SAVE:
                label _("정말로 이 파일을 삭제하시겠습니까?"):
                    style "confirm_prompt"
                    xalign 0.5
            elif message == layout.OVERWRITE_SAVE:
                label _("정말로 이 파일 위에 덮어씌우시겠습니까?"):
                    style "confirm_prompt"
                    xalign 0.5
            elif message == layout.MAIN_MENU:
                label _("정말로 메인 메뉴로 돌아가시겠습니까?"):
                    style "confirm_prompt"
                    xalign 0.5
                ...
                label _("정말로 불러오시겠습니까?"):
                    style "confirm_prompt"
                    xalign 0.5
            elif message == layout.SLOW_SKIP:
                label _("정말로 빠르게 넘기겠습니까?"):
                    style "confirm_prompt"
                    xalign 0.5
            elif message == layout.FAST_SKIP_SEEN:
                label _("정말로 다음 선택지까지 빠르게 넘기겠습니까?"):
                    style "confirm_prompt"
                    xalign 0.5
            elif message == layout.FAST_SKIP_UNSEEN:
                label _("정말로 보지 않은 대화까지 넘기겠습니까?"):
                    style "confirm_prompt"
                    xalign 0.5
            elif message == "특별한 시가 해금되었습니다.\n읽어보시겠습니까?":
                label _("특별한 시가 해금되었습니다.\n읽어보시겠습니까?"):
                    style "confirm_prompt"
                    xalign 0.5
            else:
                label _(message):

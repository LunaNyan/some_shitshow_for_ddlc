init python:
    ...
    with renpy.file('poemwords.txt') as wordfile:
        for line in wordfile:
            ...
            xline = line
            full_wordlist.append(PoemWord(xline[:-6], float(xline[-5]), float(xline[-3]), float(xline[-1])))
...
label poem(transition=True):
    ...
    if persistent.playthrough == 0 and chapter == 0:
        call screen dialog("시를 써 보자!\n\n맘에 드는 부원이 좋아할 만한 단어들을 골라 써 보자.\n내가 쓴 시를 좋아한다면\n두근 두근한 일이 생길지도?", ok_action=Return())
    python:
